...
"""TF 2.0 OpenAI ModernGPT2 model."""
...
from transformers.activations_tf import get_tf_activation # MODIFIED
from transformers.modeling_tf_outputs import ( # MODIFIED
    TFBaseModelOutputWithPastAndCrossAttentions,
    TFCausalLMOutputWithCrossAttentions,
    TFSequenceClassifierOutputWithPast,
)
from transformers.modeling_tf_utils import ( # MODIFIED
...
from transformers.tf_utils import check_embeddings_within_bounds, shape_list, stable_softmax # MODIFIED
from transformers.utils import ( # MODIFIED
    ModelOutput,
...
from transformers.models.moderngpt2.configuration_moderngpt2 import ModernGPT2Config # MODIFIED


logger = logging.get_logger(__name__)

_CHECKPOINT_FOR_DOC = "openai-community/moderngpt2"  # TODO: Update this when model is actually available
_CONFIG_FOR_DOC = "ModernGPT2Config"


class TFModernGPT2Attention(keras.layers.Layer):
        ...
        if encoder_hidden_states is not None:
            if not hasattr(self, "q_attn"):
                raise ValueError(
                    ...
                    "Please make sure to instantiate class with `TFModernGPT2Attention(..., is_cross_attention=True)`."
                )
...
class TFModernGPT2MLP(keras.layers.Layer):
...
class TFModernGPT2Block(keras.layers.Layer):
    def __init__(self, config, scale=False, **kwargs):
        super().__init__(**kwargs)
        nx = config.n_embd
        inner_dim = config.n_inner if config.n_inner is not None else 4 * nx
        self.ln_1 = keras.layers.LayerNormalization(epsilon=config.layer_norm_epsilon, name="ln_1")
        self.attn = TFModernGPT2Attention(nx, config, scale, name="attn")
        self.ln_2 = keras.layers.LayerNormalization(epsilon=config.layer_norm_epsilon, name="ln_2")

        if config.add_cross_attention:
            self.crossattention = TFModernGPT2Attention(nx, config, scale, name="crossattention", is_cross_attention=True)
            self.ln_cross_attn = keras.layers.LayerNormalization(
                epsilon=config.layer_norm_epsilon, name="ln_cross_attn"
            )

        self.mlp = TFModernGPT2MLP(inner_dim, config, name="mlp")
...
@keras_serializable
class TFModernGPT2MainLayer(keras.layers.Layer):
    ...
    def __init__(self, config, *inputs, **kwargs):
        ...
        self.h = [TFModernGPT2Block(config, scale=True, name=f"h_._{i}") for i in range(config.n_layer)]
...
@dataclass
class TFModernGPT2DoubleHeadsModelOutput(ModelOutput):
    """
...
@add_start_docstrings(
    ...
    ModernGPT2_START_DOCSTRING,
)
class TFModernGPT2Model(TFGPT2PreTrainedModel):
    def __init__(self, config, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
        self.transformer = TFModernGPT2MainLayer(config, name="transformer")

    @unpack_inputs
    @add_start_docstrings_to_model_forward(ModernGPT2_INPUTS_DOCSTRING)
    @add_code_sample_docstrings(
...
class TFModernGPT2LMHeadModel(TFGPT2PreTrainedModel, TFCausalLanguageModelingLoss):
    def __init__(self, config, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
        self.transformer = TFModernGPT2MainLayer(config, name="transformer")
...
class TFModernGPT2DoubleHeadsModel(TFGPT2PreTrainedModel):
    def __init__(self, config, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
        config.num_labels = 1
        self.transformer = TFModernGPT2MainLayer(config, name="transformer")
    ...
    @unpack_inputs
    @add_start_docstrings_to_model_forward(ModernGPT2_INPUTS_DOCSTRING)
    @replace_return_docstrings(output_type=TFModernGPT2DoubleHeadsModelOutput, config_class=_CONFIG_FOR_DOC)
    def call(
        self,
        input_ids: TFModelInputType | None = None,
        past_key_values: Optional[Tuple[Tuple[Union[np.ndarray, tf.Tensor]]]] = None,
        ...
        ```python
        >>> import tensorflow as tf
        >>> from transformers import AutoTokenizer, TFModernGPT2DoubleHeadsModel

        >>> tokenizer = AutoTokenizer.from_pretrained("openai-community/gpt2")
        >>> model = TFModernGPT2DoubleHeadsModel.from_pretrained("openai-community/gpt2")
        ...
        return TFModernGPT2DoubleHeadsModelOutput(
            logits=lm_logits,
            mc_logits=mc_logits,
            past_key_values=transformer_outputs.past_key_values,
            hidden_states=all_hidden_states,
...
class TFModernGPT2ForSequenceClassification(TFGPT2PreTrainedModel, TFSequenceClassificationLoss):
    def __init__(self, config, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
    ...
    @add_start_docstrings_to_model_forward(ModernGPT2_INPUTS_DOCSTRING)
    @add_code_sample_docstrings(
        checkpoint="microsoft/DialogRPT-updown", # TODO: Update this to a moderngpt2 checkpoint
...
__all__ = [
    ...
    "TFModernGPT2Model",
    "TFGPT2PreTrainedModel", # This one should remain as TFGPT2PreTrainedModel
]
